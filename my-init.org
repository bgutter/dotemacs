...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package uniquify)              ; rename buffers of same name intelligently
  (use-package undo-tree)             ; incredible undo/redo support
  (use-package auto-complete)         ; neat autocompletion
  (use-package framemove)             ; jump between frames and windows
  ...
  (use-package dired-x)               ; Dired improvements (distributed with emacs, but manually loaded)
  ...
  (use-package narrow-indirect)       ; Make indirect buffer in new window, then narrow to region
  ...
  (use-package fence-edit)            ; Edit regions of one buffer in another buffer (usually with different major mode)
...
#+begin_src emacs-lisp
  (let
      ((dirname (if (on-windows-p)
                    "C:/virtual-environmentsenvironments"
                  "~/python-envs/")))
