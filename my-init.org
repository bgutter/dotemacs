...
#+begin_src emacs-lisp
  ...
  (require 'org-notmuch)           ; linking to individual emails from notmuch
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  (general-define-key :prefix my:shortcut-leader
                      ...
                      "osc"              'my:org-search-concept
                      "osj"              'my:org-search-journal
                      "osg"              'my:org-search-llm-history
                      "oss"              'my:org-search
  ...
  (defun my-org-ctrl-c-ctrl-c ()
    (if (org-in-regexp "#[a-zA-Z-]+")
        (let ((search-str (replace-regexp-in-string "-" "[- ]" (substring (match-string 0) 1))))
          ...
          (ag-regexp search-str my:org-file-tree-base-path)
...
#+begin_src elisp
  ...
  (defun goto-or-create-toplevel-headline (headline-name)
    ...
    (goto-char (point-min))
    (if (re-search-forward (concat "^\\* " headline-name) nil t)
  ...
  (defun my:org-search-ag-regex (regex)
    (interactive "sEnter ag search regex: ")
    (let (results-buf)
      (save-window-excursion
        (let ((ag-command (format "ag --nocolor --nogroup --line-number --column --org -- '%s' %s"
                                 regex
                                 my:org-file-tree-base-path))
        ...
        (shell-command ag-command output-buffer)
        ;; Process output
...
[[https://magnus.therning.org/2021-03-14-keeping-todo-items-in-org-roam.html][here]]. The function to check for open tasks in the current buffer was
taken verbatim from that blog. My only change is that I'm caching the
agenda file list directly rather than using an org-roam database.
...
#+BEGIN_SRC emacs-lisp
  (defun my:org-read-agenda-index ()
    ...
    (with-temp-buffer
      (erase-buffer)
      (insert-file-contents (concat my:org-file-tree-base-path "/org-agenda-files.cache.el"))
      (setq org-agenda-files (read (current-buffer)))))

  (defun my:org-write-agenda-index ()
    "Write `org-agenda-files' to our file cache."
    (interactive)
    (with-temp-file (concat my:org-file-tree-base-path "/org-agenda-files.cache.el")
      (prin1 org-agenda-files (current-buffer))))

  (defun my:current-buffer-has-open-tasks ()
    "Test if the current buffer has open TODO items"
    (org-element-map
  ...
  (defun my:org-add-or-remove-current-buffer-agenda-index ()
    ...
    (if (and
         (string-prefix-p my:org-file-tree-base-path (buffer-file-name))
  ...
  (defun my:org-fully-rebuild-org-agenda-files ()
    ...
    (interactive)
    (dolist (file (directory-files-recursively my:org-file-tree-base-path ".*\\.org$"))
      (with-current-buffer (find-file-noselect file)
...
#+begin_src emacs-lisp
  (when my:org-file-tree-base-path
    ...
    (let ((agenda-files-cache-path (concat my:org-file-tree-base-path "/org-agenda-files.cache.el")))
...
#+BEGIN_SRC emacs-lisp
  (defun my:org-search-concept--occur (query)
      (interactive "sOccur Pattern: ")
      (let ((list-matching-lines-default-context-lines 15)
            buffers)
        (dolist (file (directory-files-recursively (concat my:org-file-tree-base-path "/concepts/") ".*" t))
          (when (file-exists-p file)
            (let ((buf (find-file-noselect file)))
              (push buf buffers))))
        (multi-occur buffers query)))

  (defun my:org-search--ag ()
    (interactive)
    (counsel-ag "" my:org-file-tree-base-path))

  (defun my:org-search-concept--ag ()
    (interactive)
    (counsel-ag "" (concat my:org-file-tree-base-path "/concepts/")))

  (defun my:org-search-journal--ag ()
    (interactive)
    (counsel-ag "" (concat my:org-file-tree-base-path "/journal/")))

  (defun my:org-search-llm--ag ()
    (interactive)
    (counsel-ag "" (concat my:org-file-tree-base-path "/llm-history/")))

  (defun my:org-search ()
    (interactive)
    (if (on-windows-p)
        (call-interactively 'my:org-search--occur)
      (call-interactively 'my:org-search--ag)))

  (defun my:org-search-concept ()
    (interactive)
    (if (on-windows-p)
        (call-interactively 'my:org-search-concept--occur)
      (call-interactively 'my:org-search-concept--ag)))

  (defun my:org-search-journal ()
    (interactive)
    (if (on-windows-p)
        (call-interactively 'my:org-search-journal--occur)
      (call-interactively 'my:org-search-journal--ag)))

  (defun my:org-search-llm-history ()
    (interactive)
    (if (on-windows-p)
        (call-interactively 'my:org-search-llm--occur)
      (call-interactively 'my:org-search-llm--ag)))
#+END_SRC
  ...
  (add-to-list
   ...
   (cons 'org-assistant
         (my:read-file-as-string
          (concat my:org-file-tree-base-path "/llm-assistant-prompt.org"))))
#+end_src
  ...
  (gptel-make-tool
   ...
   (lambda (keywords)
     (let ((keywords-str (mapconcat 'shell-quote-argument keywords "|"))
           (base-path (expand-file-name my:org-file-tree-base-path))
           (output ""))
       (with-temp-buffer
...
#+begin_src emacs-lisp
  (gptel-make-tool
   ...
   (lambda (relative-filepath)
     (my:read-file-as-string (concat my:org-file-tree-base-path "/" relative-filepath))))
#+end_src
