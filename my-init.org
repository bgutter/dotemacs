...
This is used throughout the config to create unnamed commands (no
...
#+BEGIN_SRC emacs-lisp
  ...
  (general-define-key
      ...
      ;; OS Utilities
      "H-f"  'counsel-ag
      "H-F"  (ilambda () (setq current-prefix-arg '(4))(call-interactively 'counsel-ag))
...
#+BEGIN_SRC emacs-lisp
  ...
  Prompts for save location."
    ...
    (let*
      ...
      (if (y-or-n-p (concat "Scanning to \"" captured-path "\", continue? "))
        (shell-command-to-string
          (concat
            "tmpfileraw=$(mktemp)\n"
            "tmpfilebig=$(mktemp)\n"
            "mkdir -p `dirname \"" captured-path "\"`\n"
            "utsushi scan " my:preferred-scanner " $tmpfileraw --no-interface --image-format=TIFF --resolution=250 --duplex\n"
...
#+BEGIN_SRC emacs-lisp
(defun my:reset-background-color ()
  (if (minibufferp)
...
(add-hook 'minibuffer-setup-hook #'my:reset-background-color)
(add-hook 'minibuffer-exit-hook #'my:minibuffer-focus-background-color)
#+END_SRC
...
Launch PDB for the current file, simple.
...
(defun my:simple-pdb ()
  ...
  (interactive)
  (my:call-realgud
    (realgud:pdb (pdb-suggest-invocation "pdb"))))
#+END_SRC
