...
#+begin_src elisp
  ...
  (defun my-search-hash-tag ()
    (interactive)
    (when (org-in-regexp "#[a-zA-Z-]+")
      (let* ((tag (substring (match-string 0) 1))
             (search-str (replace-regexp-in-string "-" "[- ]" tag))
             (ag-command (format "ag --nocolor --nogroup --line-number --column -- '%s' %s" search-str my:org-file-tree-base-path))
             (output-buffer (generate-new-buffer "*Org Hash Tag Results*"))
             (results-buffer (get-buffer-create "*Org Hashtag Matches*")))
    ...
    (let ((map (make-sparse-keymap)))
      ...
      (define-key map (kbd "RET") (lambda ()
                                    ...
                                    (let ((file (org-entry-get nil "SOURCE"))
                                      ...
                                      (message "Going to %s in %s" line file)
                                      (find-file-other-window file)
                                      (goto-line (string-to-number line))
                                      (recenter)
                                      (pulse-momentary-highlight-one-line (point)))))
      (define-key map (kbd "C-c C-o") (lambda () (interactive)
                                        (let ((file (car (split-string (thing-at-point 'line) ":"))))
                                          (find-file file))))
      map)
  ...
  (add-hook 'org-ctrl-c-ctrl-c-final-hook 'my-search-hash-tag)
#+end_src
