...
Most of these were written as a "learning elisp" exercise when I first
started emacs, but then I ended up actually using them
consistently. There are better approaches to this.

TOOD: Replace with yasnippet or other libraries where appropriate
...
Ignore dependencies when I explicitly want to set status.
...
#+BEGIN_SRC emacs-lisp
  ...
  (general-define-key
      ...
      "H-b"         'my:find-buffer
      "H-B"         'counsel-locate
      "H-w"         'eyebrowse-switch-to-window-config
      "H-W"         'eyebrowse-rename-window-config
  ...
  (general-define-key :keymaps 'org-agenda-mode-map
      ...
      "C-S-<down>"  (lambda () (interactive) (shrink-window 5))
...
#+BEGIN_SRC emacs-lisp
(custom-set-variables '(olivetti-body-width 130))
(custom-set-variables '(olivetti-minimum-body-width 130))
#+END_SRC
...
(defun my:current-workspace-name ()
  (let*
      ((slot (eyebrowse--get 'current-slot))
       (name (nth 2 (assoc slot (eyebrowse--get 'window-configs)))))
    (if (> (length name) 0)
        name
      (concat "W:" (number-to-string slot)))))
...
(setq-default mode-line-format
  (list
    ...
    (mode-line-status-entry (and (window-has-focus) (fboundp 'eyebrowse--get))     (format "[%s]" (my:current-workspace-name)) "Current Workspace")
...
Scroll on a per-pixel basis, like it's 1999 (rather than 1989). Based on advice on [[https://emacs.stackexchange.com/questions/10354/smooth-mouse-scroll-for-inline-images][StackOverflow]].
...
Use minted code sections in PDF export, and fixup the margins a bit.
...
Fix the inferior lisp program. Slime looks for 'lisp' by default,
which is not ever in my path.
...
7. Make sure the AutoHotkey script for Windows is *not* running. This is very important, if bouncing back and forth between emacs/WSL and EmacsForWindows.
8. Install =fonts-noto= and =fonts-liberation=, =silversearcher-ag=, and whatever else is needed.
9. Install =vcxsrv= on Windows and run it.
10. Mount network shares like =sudo mount -t drvfs '\\blah.com\SHARE' /mnt/blah=. Save any recurring mounts in ~/mount-drives.sh. TODO: fstab?
11. Save the script below as =runemacs.sh=. It will run GUI emacs in the py36_64 conda env.
13. Kindly ask IT, just one more time, to let you install native Linux. :)
