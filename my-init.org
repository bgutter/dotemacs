...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my:current-buffer-has-open-tasks ()
    ...
    (org-element-map
        ...
        'headline
      (lambda (h)
        (and (eq (org-element-property :todo-type h) 'todo)
             (not (or (member "NOAGENDA" (org-element-property :tags h))
                      (member "ARCHIVE" (org-element-property :tags h))))))
      nil 'first-match))
...
#+BEGIN_SRC emacs-lisp
  (defun my:org-search-concept--occur (query)
      ...
      (let ((list-matching-lines-default-context-lines 15)
            buffers)
        (dolist (file (directory-files-recursively (concat org-directory "/concepts/") ".*" t))
