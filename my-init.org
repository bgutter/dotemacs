...
#+BEGIN_SRC emacs-lisp
  ...
  (defcustom my:commit-org nil
    "Whether to automatically stage and commit in org-directory on org-save-all-org-buffers")
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package lsp-mode)              ; LSP support
  ...
  (use-package cape                   ; Contains a shim to get corfu and eglot working together
    ...
    (add-to-list 'eglot-server-programs
                 '((python-mode python-ts-mode) "basedpyright-langserver" "--stdio"))) ;; enabled basedpywright
  ...
  (use-package org-ql)                ; A sane interface for finding things in Org
                                      ; Cherry-pick ea72409f64d8c4e4bfa70504b527a96ee672e089 in the
                                      ; repo cloned by straight.el to fix sorting
  ...
  (use-package agent-shell           ; Coding with AI
    :custom
  ...
  (use-package org-super-agenda)      ; better agendas
  (use-package origami
    :hook ((org-agenda-mode . origami-mode)))               ; another folding mode. Works with org-super-agenda without fuss
...
More hacks for windows -- for some reason, emacs has been constantly
asking my about file encodings on this platform lately.

TODO -- surely I don't need all of this. Do I really need any of it?
...
Assign task under point

#+begin_src emacs-lisp
  (defun my:org-assign-todo ()
    "Set ASSIGNEE property for heading under point to a value using completing-read on the hashtags.ogr content"
    (interactive)
    (let ((assignee (completing-read "Assign To: " (my:org-hashtags-all))))
      (org-set-property "ASSIGNEE" assignee)))
#+end_src

And also support assignment in agenda views

#+begin_src emacs-lisp
    (defun my:org-agenda-assign ()
      "Assign task under point by setting the ASSIGNEE property."
      (interactive)
      (org-agenda-check-no-diary)
      (org-agenda-maybe-loop
       #'my:org-agenda-assign nil nil nil
       (let* ((hdmarker (or (org-get-at-bol 'org-hd-marker)
         ...
         (org-with-remote-undo buffer
           (with-current-buffer buffer
             ...
             (org-set-property "ASSIGNEE" nil)))))
      (org-agenda-redo))

  (defun my:org-agenda-unassign ()
    "Clear assignee for task under point by removing ASSIGNEE property"
    (interactive)
    (org-agenda-check-no-diary)
    (org-agenda-maybe-loop
     #'my:org-agenda-unassign nil nil nil
     (let* ((hdmarker (or (org-get-at-bol 'org-hd-marker)
                          (org-agenda-error)))
            (buffer (marker-buffer hdmarker))
            (pos (marker-position hdmarker))
...
Add, remove, and get participants

#+begin_src emacs-lisp
  (defun my:org-add-participant ()
    (interactive)
    (let* ((new-participant (completing-read "[+] Participant: " (my:org-hashtags-all)))
           (current-prop (org-entry-get (point) "PARTICIPANTS"))
           (current-participants (if current-prop (split-string current-prop) '()))
           (new-participants (delete-dups (append current-participants (list new-participant))))
           (new-prop (string-join new-participants " ")))
      (org-set-property "PARTICIPANTS" new-prop)))

  (defun my:org-remove-participant ()
    (interactive)
    (let* ((current-prop (org-entry-get (point) "PARTICIPANTS"))
           (current-participants (if current-prop (split-string current-prop) '()))
           (participant-to-pop (completing-read "[-] Participant: " current-participants))
           (new-participants (remove participant-to-pop current-participants))
           (new-prop (string-join new-participants " ")))
      (org-set-property "PARTICIPANTS" new-prop)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  (general-define-key :keymaps 'org-super-agenda-header-map
                     ...
                     "<backtab>" #'my:org-agenda-fold-sections)
  ...
  (general-define-key :prefix my:shortcut-leader
                      ...
                      ;; "od" -> Org Daily (journal actions)
                      "odd"              'my-org-journal-open-today
                      "odt"              'my-org-journal-open-tomorrow
                      "ody"              'my-org-journal-open-yesterday
                      "od?"              'my:open-journal ;; ask

                      ;; "oa" -> Org Agenda
                      "oa"               'org-agenda

                      ;; "ot" -> Org Todo
                      "ota"              'my:org-assign-todo
                      "otp"              'my:org-add-participant
                      "otP"              'my:org-remove-participant
                      "ots"              'my:org-make-sensitive
                      "otS"              'my:org-make-insensitive

                      ;; "oq" -> Org Query
                      "oqa"              'my:tasks-for-assignee-by-category
                      "oqc"              'my:tasks-for-category-by-assignee
                      "oqd"              'my:tasks-closed-recently
                      ...
                      ;; "ol" -> Org Link
                      "ols"              'org-store-link
                      "oli"              'org-insert-last-stored-link
                      "olt"              'my:org-link-and-backlink-to-open-task
                      ...
                      ;; "oh" -> Org Hashtag
                      "ohqt"             'my:org-hashtag-query-tag-under-point
  ...
  (general-define-key :keymaps 'org-agenda-mode-map
                      "&"           'my:org-ql-toggle-redact-sensitive-content
                      "p"           'org-agenda-priority
                      "t"           'my:org-agenda-todo-and-redo
                      "a"           'my:org-agenda-assign
                      "u"           'my:org-agenda-unassign
  ...
  (general-define-key :keymaps 'agent-shell-mode-map
                      "C-<up>"   'my:scroll-down-some-lines
                      "C-<down>" 'my:scroll-up-some-lines)
...
| NEXT      | Items which are considered to be in-progress. Max 3-5 items at any point in time per assignee.   |
| WAIT      | Items which are blocked and in a waiting state.                                                  |
|-----------+--------------------------------------------------------------------------------------------------|
...
| BACKLOG     | Before ASSIGNED property. Items which are queued to be completed, by someone -- maybe me. |
| ASSIGNED    | Before ASSIGNED property. Items which are queued to be completed by someone else.         |
|-------------+-------------------------------------------------------------------------------------------|
...
#+begin_src emacs-lisp
  ...
  (setq org-agenda-custom-commands
      '(("x" "Combined Task List"
         ...
         ;; WAIT -- assigned or unassigned
         ((org-ql-block '((and (todo "WAIT")
                               (not (assigned))
                               (not (tags "NOAGENDA" "ARCHIVE" "UNFILED" "milestone")))
                          :sort (deadline priority))
                        ((org-ql-block-header "Personally Blocked (Unassigned and keyword=WAIT)")))
          ...
          (org-ql-block '((and (todo "NEXT")
                              ...
                              (not (tags "NOAGENDA" "ARCHIVE" "UNFILED" "milestone")))
                          :sort (deadline priority))
                        ((org-ql-block-header "Doing Now (Unassigned and keyword=NEXT)")))

          ;; TODO -- unassigned -- Either Priority #A or Upcoming
          (org-ql-block '((and (todo "TODO")
                              (not (assigned))
                              (or (priority "A")
                                  (deadline auto))
                              (not (tags "NOAGENDA" "UNFILED" "ARCHIVE" "milestone")))
                          :sort (deadline priority))
                        ((org-ql-block-header "Personal Priority Backlog (Unassigned with Deadline Warning or #A)")))

          ;; TODO|NEXT -- assigned
          ;; Subdivided by ASSIGNEE
          (org-ql-block '((and
                           (todo "TODO" "NEXT" "WAIT")
                           (assigned)
                           (not (tags "NOAGENDA" "UNFILED" "ARCHIVE" "milestone")))
                          :sort (deadline priority))
                        ((org-ql-block-header "Assigned Tasks (No Filters)")
                         (org-super-agenda-groups '((:auto-property "ASSIGNEE")))))

          ;; TODO -- unassigned -- Priority <= #B AND either no deadline or not Upcoming
          ;; Subdivided by Category
          (org-ql-block '((and (todo "TODO")
                               (not (assigned))
                               (priority "B" "C" "D" "E")
                               (or (not (deadline))
                                   (and (not (deadline auto))
                                        (not (tags "hide_before_warn"))))
                               (not (tags "NOAGENDA" "UNFILED" "ARCHIVE" "milestone")))
                          :sort (deadline priority))
                        ((org-ql-block-header "Personal Non-Priority Backlog (Unassigned, <= #B, no deadline)")
                         (org-super-agenda-groups '((:auto-category t)))))))

        ;; Premise -- when a task has a deadline, I won't see it in backlog. I won't see it AT ALL
        ;; until it hits the warning period (which I set for each task as I set a deadline). If I ever
        ;; want to see tasks which have deadlines but are not yet in warning period, I can use the future agenda.
        ("f" "Future"
         ((org-ql-block '((and (todo "TODO" "WAIT" "NEXT")
                               (not (assigned))
                               (deadline)
                               (not (deadline auto)))
                          :sort (deadline priority))
                        ((org-ql-block-header "Unassigned Tasks with Deadline but no Warning")
                         (org-super-agenda-groups '((:auto-category t)))))))

        ("m" "Milestones"
         ((org-ql-block '((and (tags "milestone")
                               (todo "TODO")
                               (not (tags "NOAGENDA" "UNFILED" "ARCHIVE"))
                               (priority "A"))
                          :sort (deadline))
                        ((org-ql-block-header "Upcoming Key Milestones")))

          (org-ql-block '((and (tags "milestone")
                               (todo "TODO" "DONE")
                               (not (tags "NOAGENDA" "UNFILED" "ARCHIVE")))
                          :sort (deadline))
                        ((org-ql-block-header "All Project Milestones")
                         (org-super-agenda-groups '((:auto-category t)))))))))
#+end_src

Automatically collapse all org-super-agenda groups

Define the function

#+begin_src emacs-lisp

  (defun my:org-agenda-fold-sections ()
    "Call origami-close-node on every line starting with whitespace then ASSIGNEE:, or, whitespace then Category:"
    (interactive)
    (save-excursion
      (goto-char (point-min))
      (while (not (eobp))
        (when (looking-at "^[[:blank:]]+\\(ASSIGNEE:\\|Category:\\)")
...
#+begin_src emacs-lisp
;(add-hook 'org-agenda-finalize-hook #'my:org-agenda-fold-sections)
#+end_src
...
#+begin_src emacs-lisp
  (setq org-agenda-deadline-faces
           '((1.0 . (:foreground "white" :background "dark red" :weight bold))
             (0.0 . (:foreground "salmon" :slant italic))))
#+end_src
...
#+begin_src emacs-lisp
(customize-set-variable 'org-agenda-remove-tags nil)
#+end_src

When a task has a :SENSITIVE: property value t, replace it with unicode dot characters in agendas, unless =my:org-ql-hide-sensitive-results= is non-nil

#+begin_src emacs-lisp
  (defvar my:org-ql-hide-redact-results t
    "When non-nil, org-ql buffers will redact headline content for any headings with property :SENSITIVE: set to t.")

  (defun my:org-ql-view--format-element--redact-sensitive (orig-fun element)
    "When `my:org-ql-hide-sensitive-results' is non-nil, for any headings with property `:SENSITIVE:' set to `t',
  replace every non-whitespace character in the headline text with unicode dot."
    (let* ((marker (and element (or (org-element-property :org-hd-marker element)
                                    (org-element-property :org-marker element))))
           (sensitive (and my:org-ql-hide-redact-results
                           marker
                           (org-with-point-at marker
                                                (org-entry-get (point) "SENSITIVE")))))
      (if (and sensitive (org-element-property :raw-value element))
          (let* ((redacted (replace-regexp-in-string "[^ \t\n]" "•" (org-element-property :raw-value element)))
                 (new-element (list (car element)
                                    (plist-put (copy-sequence (cadr element)) :raw-value redacted))))
            (funcall orig-fun new-element))
        (funcall orig-fun element))))

  (defun my:org-ql-toggle-redact-sensitive-content ()
    "Just flip the value of `my:org-ql-hide-sensitive-results'"
    (interactive)
    (setq my:org-ql-hide-redact-results (not my:org-ql-hide-redact-results))
    (org-agenda-redo)
    (if my:org-ql-hide-redact-results
        (message "Hiding sensitive content in agenda.")
      (message "Showing sensitive content in agenda.")))
#+end_src

Install advice

#+begin_src emacs-lisp
  (advice-add 'org-ql-view--format-element :around #'my:org-ql-view--format-element--redact-sensitive)
#+end_src

Functions to set and unset sensitive for a heading

#+begin_src emacs-lisp
  (defun my:org-make-sensitive ()
    (interactive)
    (org-set-property "SENSITIVE" "t"))

  (defun my:org-make-insensitive ()
    (interactive)
    (org-delete-property "SENSITIVE"))
#+end_src

Show past week and next two in agenda view.

Also -- don't show trailing whitespace here.
...
Highlight #firstname-lastname tags.
...
Define an org-ql predicate to check assignee. Taken almost verbatim from the org-ql pedicate tutorial.

#+begin_src emacs-lisp
  (org-ql-defpred assigned (&rest names)
    "Is this item assigned to a given person? With no args, evaluates to true for any assignee."
    :body (and (property "ASSIGNEE")
               (or (null names)
                   (cl-loop for name in names thereis (property "ASSIGNEE" name)))))
#+end_src

Another to test for items in journals versus concept files

#+begin_src emacs-lisp
  (org-ql-defpred concept (&rest concept-path-items)
    "Is this item in a concept of a certain name? With no args, matches any concept file."
    :body (and (path (expand-file-name "concepts" org-directory))
               (or (null concept-path-items)
                   (cl-loop for path-item in concept-path-items thereis (path (concat "." (regexp-quote path-item) ".*"))))))
#+end_src

Another that can be used to find mentions of certain hashtags

#+begin_src emacs-lisp
  (org-ql-defpred mentions-any (&rest hashtags)
    "String-based seach across all parts of a heading and body. Any one can be found."
    :body (cl-loop for hashtag in hashtags thereis
                   (regexp (concat "\\_<" (regexp-quote hashtag) "\\_>"))))

  (org-ql-defpred mentions-all (&rest hashtags)
    "String-based seach across all parts of a heading and body. All must be found."
    :body (cl-loop for hashtag in hashtags always
                   (concat "\\_<" (regexp-quote hashtag) "\\_>")))
#+end_src

A function that will find mentions of hashtag at point

#+begin_src emacs-lisp
  (defun my:org-hashtag-query-tag-under-point (search-all)
    "Find open TODOs assigned to this tag. With prefix, find all headings with any mention of this tag."
    (interactive (list (not (null current-prefix-arg))))
    (when (org-in-regexp "#[a-zA-Z-]+")
      (let* ((tag (substring (match-string 0) 1))
             (search-files (if search-all
                               (directory-files-recursively org-directory  ".*\\.org$")
                             org-agenda-files))
             (search-str (substring-no-properties tag))
             (tag-name (concat "#" search-str)))
        (org-ql-search search-files
          (if search-all
              `(mentions-any ,search-str)
            `(and (or (done) (todo)) (mentions-any ,search-str)))
          :sort '(deadline priority todo)
          :super-groups `((:name ,(concat "Assigned to " tag-name)
                                 :and (:property ("ASSIGNEE" ,tag-name)
                                                 :todo ("TODO" "NEXT" "WAIT")))
                          (:name ,(concat "Other Open Tasks Mentioning " tag-name)
                                 :todo ("TODO" "NEXT" "WAIT"))
                          (:name ,(concat "Recently Completed By " tag-name)
                                 :and (:property ("ASSIGNEE" ,tag-name)
                                                 :todo ("DONE" "CANCELLED" "HIATUS" "MIGRATED")))
                          (:name ,(concat "Recently Completed & Mentioning " tag-name)
                                 :todo ("DONE" "CANCELLED" "HIATUS" "MIGRATED")))))))
#+end_src

Helper function to find all unique categories

#+begin_src emacs-lisp
  (defun my:list-unique-agenda-categories ()
    "List all unique categories across all agenda files using org-ql."
    (interactive)
    (let ((categories (org-ql-query
                        :select '(org-get-category)
                        :from org-agenda-files))
          unique-categories)
      (dolist (category categories)
        (unless (member category unique-categories)
          (push category unique-categories)))
      unique-categories))
#+end_src

Function to find all tasks assigned to a person, broken by category

#+begin_src emacs-lisp
  (defun my:tasks-for-assignee-by-category (assignee)
    (interactive
     (list (completing-read "Tasks Assigned To: " (my:org-hashtags-all))))
...
#+begin_src emacs-lisp
  (defun my:tasks-for-category-by-assignee (category)
    (interactive
     (list (completing-read "Tasks in Category: " (my:list-unique-agenda-categories))))
    (org-ql-search org-agenda-files
      `(and (or (todo) (done))
            (category ,category))
      :sort '(date)
      :super-groups '((:auto-property "ASSIGNEE"))))
#+end_src

Tasks completed in past three weeks

#+begin_src emacs-lisp
  (defun my:tasks-closed-recently ()
    (interactive)
    (let ((org-super-agenda-date-format "%A, %Y-%m-%d"))
      (org-ql-search (org-agenda-files)
        '(and (todo "DONE" "CANCELLED" "MIGRATED" "HIATUS")
              (closed :from -21))
        :title "Recently Completed Tasks Grouped by Date"
        :sort '(date)
        :super-groups '((:auto-ts t)))))
#+end_src

Custom link type to show a user's tasks across all agenda files

#+begin_src emacs-lisp
  (org-link-set-parameters "tasks-assigned-to"
                         :follow (lambda (assignee)
                                   (my:tasks-for-assignee-by-category assignee)))
#+end_src

And the inverse

#+begin_src emacs-lisp
  (org-link-set-parameters "tasks-in-category"
                         :follow (lambda (category)
                                   (my:tasks-for-category-by-assignee category)))
#+end_src

And a more generalized variant. Links should be like;

[ [org-ql-search-agenda:'(and (todo) ...)::super-groups:'(...)::sort:(...)][] ]

super-groups and sort are just two examples. They can be any number (0
or many) of keyword args to org-ql-search, in any order.

Unlike the link included in org-ql, this has two main variations;
- Searches agenda files, not current buffer
- Evaluates values as elisp, so they must be quoted, but can be programmatic
- Allows supplying kwargs to org-ql-search by submitting key:val pairs delimited by ::

#+begin_src emacs-lisp
  (defun my:split-string-once (string delimiter)
    "Split STRING into a list of two substrings at the first occurrence of DELIMITER."
  ...
  (org-link-set-parameters "org-ql-search-agenda"
                           :follow (lambda (link)
                                     "Form: query::keyword:(arg-form)::keyword2:(argform2)..."
                                     (let* ((parts (split-string link "::"))
                                            (query (eval (read (car parts))))
                                            (kwargs-strs (cdr parts))
                                            (kwargs nil))
                                       (mapcar
                                        (lambda (str)
                                          (pcase-let
                                              ((`(,first-part ,second-part) (my:split-string-once str ":")))
                                            (push (eval (read second-part)) kwargs)
                                            (push (intern (concat ":" first-part)) kwargs)))
                                        kwargs-strs)
                                       (apply #'org-ql-search org-agenda-files query kwargs))))
#+end_src

A helper function to support interactive selection of org-mode TODO headings

#+begin_src emacs-lisp
  (defun my:org-ql-completing-read-for-query (prompt files query)
    "Returns a marker after asking user to select the result of an org-ql query."
    (let* ((selection
            (completing-read
             prompt
             (org-ql-select files query
               :action (lambda ()
                         (let* ((todo     (org-entry-get nil "TODO"))
                                ...
                                (todo-str     (when todo
                                                (concat (propertize todo 'face (org-get-todo-face todo)) " ")))
                                (priority-str (when priority
                                                (concat (propertize (format "[#%s]" priority) 'face 'org-priority) " ")))
                                (display  (concat todo-str priority-str heading)))
                           (propertize display 'org--marker (copy-marker (point))))))
             nil
             t))
           (marker (get-text-property 0 'org--marker selection)))
      marker))
#+end_src

A helper function to append to the body of an org-mode heading

#+begin_src emacs-lisp
  (defun my:org-append-to-body (marker content)
    "Append a string to the end of the body of the org-heading at marker."
    (save-excursion
...
A helper function to get a link to the heading at a given marker

#+begin_src emacs-lisp
  (defun my:org-link-from-marker (marker &optional custom-prefix)
    "Return a string link to some marker in a heading."
    (save-excursion
      (with-current-buffer (marker-buffer marker)
        (goto-char marker)
        (org-store-link nil t)
        (let*
            ((link-entry (car org-stored-links))
             (link (car link-entry))
             (desc (cadr link-entry))
             (link-str (format "[[%s][%s]]" link (if custom-prefix (concat custom-prefix desc) desc))))
          link-str))))
#+end_src

A function to link to an open TODO, and, backlink to current heading.

#+begin_src emacs-lisp
  (defun my:org-link-and-backlink-to-open-task ()
    (interactive)
    (let ((marker (my:org-ql-completing-read-for-query "Link to Task: " (org-agenda-files) '(todo)))
          (custom-desc (read-string "Link Description: ")))
      (org-store-link nil t)
      (let*
          ((link-entry (car org-stored-links))
           (link (car link-entry))
           (desc (cadr link-entry))
           (link-str (format "[[%s][%s: %s]]" link custom-desc desc)))
        (when (markerp marker)
          (my:org-append-to-body marker link-str)
          (insert (my:org-link-from-marker marker "Linked Task: "))))))
#+end_src
...
Also make sure that org links are always performed using IDs, so that
they are not constantly breaking when moving headings around.
...
#+BEGIN_SRC emacs-lisp
  (defcustom my:org-agenda-forced-files nil
    "List of files always included in org-agenda-files, regardless of the caching logic below.")

  (defun my:org-read-agenda-index ()
    "Read cached value for `org-agenda-files'"
    (interactive)
    (let
        ((my-agenda-files-cache-path (concat org-directory "/org-agenda-files.cache.el")))
      (if (file-exists-p my-agenda-files-cache-path)
          (with-temp-buffer
            (erase-buffer)
            (insert-file-contents my-agenda-files-cache-path)
            (setq org-agenda-files (delete-dups
                                    (append (read (current-buffer))
                                            my:org-agenda-forced-files)))))))
  ...
  (defun my:org-write-agenda-index ()
    ...
    (let
        ((my-agenda-files-cache-path (concat org-directory "/org-agenda-files.cache.el")))
      (if (file-exists-p my-agenda-files-cache-path)
          (with-temp-file my-agenda-files-cache-path
            (prin1 (delete-dups org-agenda-files) (current-buffer))))))
  ...
  ;; Go ahead and read the agenda index
...
*Hashtags.org* -- maintain a set of all know #hashtag-things so that
autocomplete always works for them, even if they aren't in
org-agenda-files.

#+begin_src emacs-lisp
  (defcustom my:org-hashtags-file (concat org-directory "/hashtags.org")
    "Path to a file storing the set of all unique hashtags in org-directory"
    :type 'file
    :group 'org)

  (defun my:extract-hashtags-from-buffer ()
    "Extract all unique hashtags from buffer."
    (save-excursion
      (goto-char (point-min))
      (let ((tags '())
  ...
  (defun my:sync-hashtags-on-save ()
    "Extract hashtags in current buffer, merge with master list, sort alphabetically, and save to hashtags file."
    ;; Do not attempt to sync when saving the master hashtags file iteself
    (when (and (eq major-mode 'org-mode)
               (buffer-file-name)
               (not (file-equal-p (buffer-file-name) (expand-file-name my:org-hashtags-file))))
      (let ((buffer-tags (my:extract-hashtags-from-buffer)))
        (when buffer-tags
          (let* ((target-file   (expand-file-name my:org-hashtags-file))
                 (existing-tags (if (file-exists-p target-file)
                                    (with-temp-buffer
                                      (insert-file-contents target-file)
                                      (split-string (buffer-string) "\n" t "\\s-+"))
                                  '()))
                 (all-tags (sort (delete-dups (append buffer-tags existing-tags))
                                 #'string-collate-lessp))
                 (new-count (- (length all-tags) (length existing-tags))))
            (when (> new-count 0)
              (with-temp-file target-file
                (insert (string-join all-tags "\n") "\n"))
              (message "Found %s new hashtags." new-count)))))))

  (defun my:org-fully-cache-hashtags ()
    "Manually rescan and rebuild the org hashtag index"
    (interactive)
    (dolist (file (directory-files-recursively org-directory ".*\\.org$"))
      (with-current-buffer (find-file-noselect file)
        (my:sync-hashtags-on-save))))

  (add-hook 'org-mode-hook
            (lambda ()
              (add-hook 'before-save-hook #'my:sync-hashtags-on-save nil t)))

  (defun my:read-lines-from-file (file-path)
    "Read lines from file and return as list of strings"
    (with-temp-buffer
      (insert-file-contents file-path)
      (split-string (buffer-string) "\n" t)))

  (defun my:org-hashtags-all ()
    (my:read-lines-from-file my:org-hashtags-file))
#+end_src

Add org C-c C-c handler for hsahtags that calls =my:org-hashtag-query-tag-under-point=.

#+begin_src emacs-lisp
  (add-hook 'org-ctrl-c-ctrl-c-final-hook
            (lambda ()
              (call-interactively #'my:org-hashtag-query-tag-under-point)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (defun my:org-autocommit-get-message ()
    "Return a string for an org autocommit commit message.
     Format: YYYY-MM-DD HH:MM am/pm (SS-ms)"
    (prin1-to-string
     (string-join
      (list
       (downcase (format-time-string "%Y-%m-%d %I:%M %p (%S-%3N)"))
       ""
...
Support for evaluating an init.org present in root of =org-directory=.

#+begin_src emacs-lisp
  (when (boundp 'org-directory)
    (let ((org-init-file (expand-file-name "init.org" org-directory)))
