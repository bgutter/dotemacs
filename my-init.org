...
- *use-frames*: Whether or not to tune use frame-oriented options in
  emacs. Nil by default, set to t on systems with tiling window
  managers.
...
(package-require 'helm)                  ; Only need a few things, have mostly moved to ivy
...
Stop trying to make tabs happen. They're not going to happen.
...
When use-frames is set, configure emacs to use popup frames. Also,
...
(setq-default org-todo-keywords
  '((sequence "TODO(t!)" "IN_PROGRESS(i!)" "NEXT(n!)" "HIATUS(h!)" "PENDING(p!)" "WAIT(w!)" "|" "DONE(d!)" "CANCELLED(c!)" "MIGRATED(m!)")))
#+END_SRC
...
(custom-set-faces
  '(org-block-begin-line
     ((t (:weight bold :foreground "#FFC0CB" :background "#222222"))))
  '(org-block
     ((t (:background "#222222"))))
  '(org-block-end-line
     ((t (:weight bold :foreground "#FFC0CB" :background "#222222")))))
...
(defun my-org-update-agenda-index()
  (interactive)
  (setq org-agenda-files (directory-files-recursively my:org-file-tree-base-path ".*\\.org$")))
...
Place helm completions in a temp window at the bottom of the frame,
period. Thankfully, ivy doesn't need this. Source on [[https://github.com/wasamasa/shackle/issues/28][Github]].
