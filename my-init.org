...
#+begin_src emacs-lisp
  (defun my:org-add-participant ()
    (interactive)
    (let* ((new-participant (completing-read "[+] Participant: " org-roles-known-handles))
...
#+begin_src emacs-lisp
  (defun my:tasks-for-assignee-by-category (assignee)
    (interactive
     (list (completing-read "Tasks Assigned To: " org-roles-known-handles)))
    ...
    (let ((marker (my:org-ql-completing-read-for-query "Link to Task: " (org-agenda-files) '(todo)))
      ...
      (let*
          ((link-entry (car org-stored-links))
           (link (car link-entry))
           (desc (cadr link-entry))
           (link-str (format "[[%s][🔗 %s ]]" link custom-desc))) ;; spaces enable braces in link desc
        (when (markerp marker)
          (my:org-append-to-body marker link-str)
          (insert (my:org-link-from-marker marker "✅ Task Mention: "))))))
#+end_src
