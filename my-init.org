...
Append to an environment variable (unlixlike).
...
#+BEGIN_SRC emacs-lisp
  (defun make-easing-calculator (easing-period-seconds reset-period-seconds &optional throttle-period-seconds max-step-size)
    ...
    (lexical-let ((start-time nil)
      ...
      (lambda ()
        (let* ((now         (float-time))
               ...
               ;; (speed       (* speed speed))
               (distance    (max 1 (* speed max-step-size))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq move-frame-easing-calc   (make-easing-calculator 0.5 0.15 0.01 47))
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq resize-frame-easing-calc (make-easing-calculator 0.5 0.15 0.03 8))
